function(input, output, session) {
 ...
 # clicking updates selectInput
  ...
  townreac <- reactive(MAtowndata[MAtowndata$Town==input$town,])
  ...
  observeEvent(input$town, {
    leafletProxy("map", session) %>%
      ...
      addPolygons(data=MAtown[MAtown$TOWN==townreac()$Town,], weight = 1, color="Red", fillColor="Yellow",fillOpacity = 1, group="selectedTownShp")
    ...
    if (input$town == " "){
      output$tabledemog <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town=="Boston",c(1,5,4)]),
                                               caption = demogcaption,
                                               options = list(
                                                 dom="t",
                                                 columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                 headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))
      output$tableest <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town=="Boston",c(9:12,8,15,16)]),
                                             caption = estcaption,
                                             options = list(dom="t",
                                                            columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                            headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))
      ...
      output$tabledemog <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town==townreac()$Town,c(1,5,4)]),
                                               caption = demogcaption,
                                               options = list(dom="t",
                                                              columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                              headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))
      output$tableest <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town==townreac()$Town,c(9:12,8,15,16)]),
                                             caption = estcaption,
                                             options = list(dom="t",
                                                            columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                            headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))
  ...
  output$downloadData <- downloadHandler(
    filename = "Air-Pollution-MA.csv",
    content = function(file) {
      write.csv(MAtowndata, file)
    }
  )
  ...
  output$tabledemogleft <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town==secondpageinput()[1],c(1,4,5)]),
                                               caption = demogcaption,
                                               options = list(dom="t",
                                                              columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                              headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))
  
  output$tablepoprateleft <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town==secondpageinput()[1],c(19:20,17:18)]),
                                             caption = popratecaption,
                                             options = list(dom="t",
                                                            columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                            headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))
 
  output$tableIQleft <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town==secondpageinput()[1],c(15:16)]),
                                            caption = IQcaption,
                                            options = list(dom="t",
                                                           columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                           headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))
  
  output$tabledemogright <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town==secondpageinput()[2],c(1,4,5)]),
                                                caption = demogcaption,
                                                options = list(dom="t",
                                                               columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                               headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))
  
  output$tablepoprateright <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town==secondpageinput()[2],c(19:20,17:18)]),
                                              caption = popratecaption,
                                              options = list(dom="t",
                                                             columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                             headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))

  output$tableIQright <- DT::renderDataTable(t(MAtowndata[MAtowndata$Town==secondpageinput()[2],c(15:16)]),
                                                caption = IQcaption,
                                                options = list(dom="t",
                                                               columnDefs = list(list(className = 'dt-right', targets = 1)),
                                                               headerCallback = JS("function(thead, data, start, end, display){$(thead).remove();}")))
